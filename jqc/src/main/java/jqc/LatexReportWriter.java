...
public class LatexReportWriter {
    ...
    /**
     * Try to run pdflatex from the command line to generate a PDF file
     * automatically. */
	...
	out.write("\\documentclass[10pt]{article}\n" +
		  ...
		  "\\begin{document}\n" +
		  "\\section*{Exome Q/C Report: "+fname+"}\n\n");
	...
	if (n_samples > 1) {
	    out.write("There were " + n_samples + " samples and a total of " +
		      n_totalVariants + " identified variants.\n");
	} else {
	     out.write("There were a total of " +
		      n_totalVariants + " identified variants.\n");
	}
    }
	...
	String off = String.format("%.1f",100*(double)n_offTarget/ n_total);
	 out.write("\\begin{itemize} \\item Variants were filtered to be with " + 
		   distanceThreshold + " nucleotides of an exon. " + n_offTarget + " off-target variants (" +
		   off + "\\%) were " +
	...
	out.write("This Quality Control report was generated " + 
		  " using JQCVCF from the Jannovar project. ");
